...
void DNE::runDNE(const int& MxEdges,const TStr& InferredFNm,int snapshot) {
		...
		for (int i = CascV[c].Len() - 1; i >=0 ; i--) {	
			for(int j = 0 ;j < i;j++) {
				...
				if (CascV[c].GetTm(src) < CascV[c].GetTm(dst)){
					ranks[src][dst] += rankCoef1/((double)(CascV[c].Len() - 1 - j) * (i - j)) ;
					potentialEdges ++;
					numCascades[src][dst] ++;						
				}								
			}
	...
	if (snapshot == 1) 
	{
		...
		for(int y=0;y<ch;y++)
		{
			if(!inferredNetwork->IsEdge(weights[y].start,weights[y].end,true)){

				inferredNetwork->AddEdge(weights[y].start,weights[y].end);
			}
		}


		// write nodes to file
		for (TNGraph::TNodeI NI = inferredNetwork->BegNI(); NI < inferredNetwork->EndNI(); NI++) {
			fprintf(inferredFOut,"%d,%d\r\n", NI.GetId(), NI.GetId());
		}

		fprintf(inferredFOut,"\r\n");
...
void DNE::LoadCascadesTxt(TSIn& SIn) {
	CascV.Clr(); 
	TStr Line;
	SIn.GetNextLn(Line);
	while (!SIn.Eof() && Line != "") {
...
void DNE::LoadLastGroundTruthTxt(TSIn& SIn){
	...
	while (!SIn.Eof() && Line != "") {
		TStrV NIdV;
		Line.SplitOnAllCh(',', NIdV);
		LastGroundTruth->AddNode(NIdV[0].GetInt());
		SIn.GetNextLn(Line);
	}
	// add edges
	while (!SIn.Eof()) {
...
void DNE::AddCasc(const TStr& CascStr, const double& alpha) {
	TStrV NIdV_temp, NId;
	CascStr.SplitOnAllCh(';', NIdV_temp);
	NIdV_temp[1].SplitOnAllCh(',',NId);
	TCascade C;
...
void DNE::InitializeVariables(const bool mode, const int nodes) {
	if(mode) {
		...
		transition_00_num = 0; transition_01_num = 0; transition_10_num = 0; transition_11_num = 0;
		for (int i = 0; i < nodes; i++) {
...
void DNE::ExtractParameter_Transition(TSIn& pastFile, TSIn& presentFile, const int nodes, int snapShot) {	
	...
	while(!presentFile.Eof()) {
		...
		if(check_present == 1) {	
			...
			i_prime = presentNIdV[0].GetInt(); 
			j_prime = presentNIdV[1].GetInt();
...
void DNE::SaveParameter_Transition(const TStr& OutFNm, const int nodes,const char* address){
	TFOut FOut(OutFNm);
...
void DNE::CalculateParameter_Emission(const TStr& rankFNm, const TStr& networkFNm, const int NSnapshot) {
	...
	for(int ts = 1; ts <= NSnapshot; ts++) {
		...
		// Calculate emission (Input: network edges, network ranks)

		num_one = 0; num_zero = 0;
		for(int i = 0; i < nodes; i++){
			for(int j = 0; j < nodes; j++){
				if(GroundTruth->IsEdge(i,j)){
					emission[1][linkRanks[i][j]]++;
					num_one++;

				}
				else if(!GroundTruth->IsEdge(i,j)) {
...
void DNE::compare(double Threshold, const TStr& ResultFNm, const TStr& e_probFNm, const TStr& InferredFNm, const int MxEdges, int snapShot, int prior) { 
	...
	loadEmissionProbability(e_probFNm.CStr()); 
	...
	for(int i = 0; i < nodes; i++) {
		for(int j = 0; j < nodes; j++) {
			...
			if(LastGroundTruth -> IsEdge(i,j)) {
				if(emissionProbability[1][rank] == 0) { // This emission value has not been seen in the training snapshot					
					int yu = (rank) - 1;
					while (yu >= 0){
						if(emissionProbability[1][yu] > 0) { // Set the default emission value of this rank to the emission value of nearset smaller seen rank						
							probability[i][j] = ((transition_11[i][j] * emissionProbability[1][yu]) / ((transition_11[i][j] * emissionProbability[1][yu]) + (transition_10[i][j] * emissionProbability[0][yu]) ) );
							break;
						}
						yu--;
					...
					probability[i][j] = ((transition_11[i][j] * emissionProbability[1][rank]) / ((transition_11[i][j] * emissionProbability[1][rank]) + (transition_10[i][j] * emissionProbability[0][rank]) ) );
			...
			if(!LastGroundTruth -> IsEdge(i, j)) {
				...
				if(emissionProbability[1][rank] == 0) {	// This emission value has not been seen in the training snapshot					
					int yu =(rank) - 1;
					while(yu >= 0) {
						if(	emissionProbability[1][yu] > 0) { // Set the default emission value of this rank to the emission value of biggest smaller seen rank
							probability[i][j] = ( (transition_01[i][j] * emissionProbability[1][yu]) / ((transition_01[i][j] * emissionProbability[1][yu]) + (transition_00[i][j] * emissionProbability[0][yu])));
							break;
						}
						yu--;
					}
				}else{
					probability[i][j] = ((transition_01[i][j] * emissionProbability[1][rank]) / ((transition_01[i][j] * emissionProbability[1][rank]) + (transition_00[i][j] * emissionProbability[0][rank])));
	...
	for(int i = 0; i < nodes; i++) {
		for(int j = 0; j < nodes; j++) {
			if(probability[i][j] > Threshold) { //041

				if(!inferredNetwork->IsEdge(i,j,true)) {
					inferredNetwork->AddEdge(i,j);	
	...
	for (int i = 0; i < PrecisionRecall.Len(); i++) {
		...
		if(i == PrecisionRecall.Len() - 1) {
			...
			printf("fmeasure : %f \n ", fmeasure);
			cout << inferredNetwork->GetEdges() << "\t" << GroundTruth->GetEdges() << endl;
		}
	}
...
void DNE::loadEmissionProbability(const TStr& e_probFNm) {
	...
	// Computing Graphical Model Probability
	...
	int x1_num, x0_num, t_num; x1_num = 0; x0_num = 0;
	for(int i=0; i < 2; i++){
...
void DNE::UpdateTransitionProbability(const TStr& t_probFNm, const TStr& t_newProbFNm, int i){ //,const char* address) {
	...
	TStr Line; TStrV NIdV2, NIdV3, NIdvm;
	...
	for(int i = 0; i < nodes; i++) {
		for(int j = 0; j < nodes; j++) {
			...
			if(LastGroundTruth -> IsEdge(i, j) && inferredNetwork -> IsEdge(i, j)) {
				tr_11++;
			}else if(!LastGroundTruth -> IsEdge(i, j) && inferredNetwork -> IsEdge(i, j)) {
				tr_01 += sFactor;
				count_0 += sFactor;
			}
		}
