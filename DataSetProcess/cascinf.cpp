...
void TNIBs::LoadCascadesRealTxt(TSIn& SIn) {
	TStr Line;
	int y = 1;
	printf("salam");
	while (!SIn.Eof()) {
               ...
               if (Line=="") { break; }
                TStrV NIdV; Line.SplitOnAllCh(',', NIdV);
	        AddNodeNm(NIdV[0].GetInt(), TNodeInfo(NIdV[0], 0)); }
		printf("All nodes read!\n");
	while (!SIn.Eof()) {
		 SIn.GetNextLn(Line);
		 AddRealCasc(Line, Model, (y/5000));
		if(y%5000 == 0)
			 printf("%d cascades read\n", y);
		 y++;
	}
	printf("number of lines: %d",y);
	printf("All cascades read!\n");
}
...
void TNIBs::AddRealCasc(const TStr& CascStr, const TModel& Model, int snapShot) {
    int CId = CascH.Len();
    TStrV NIdV, NId_t; 
    CascStr.SplitOnAllCh(';', NIdV);
    TCascade C(CId, Model);
    for (int i = 0; i < NIdV.Len(); i++) {
      NIdV[i].SplitOnAllCh(',',NId_t);
      int NId;
      double Tm;
      NId = NId_t[0].GetInt();
      Tm = NId_t[1].GetFlt() + (100 * snapShot);
